...
public class UserReadingForm extends JDialog {
    ...
    public UserReadingForm(MainForm mainForm) {
        ...
        txtName.addKeyListener(new KeyAdapter() {
            // Sobrescribe el metodo keyReleased, que se llama cuando se suelta una tecla.
            @Override
            public void keyReleased(KeyEvent e) {
                // Verifica si el campo de texto txtNombre no está vacío.
                if (!txtName.getText().trim().isEmpty()) {
                    // Llama al metodo search para buscar usuarios según el texto ingresado.
        ...
        btnUpdate.addActionListener(s -> {
            // Llama al metodo getUserFromTableRow para obtener el usuario seleccionado en la tabla.
    ...
    private void search(String query) {
        try {
            // Llama al metodo 'search' del UserDAO para buscar usuarios cuya información
            // coincida con la cadena de búsqueda 'query'. La implementación específica
            ArrayList<User> users = userDAO.search(query);
            // Llama al metodo 'createTable' para actualizar la tabla de usuarios
            ...
            return; // Sale del metodo 'search' después de mostrar el error.
        }
    }
    public void createTable(ArrayList<User> users) {

        // Crea un nuevo modelo de tabla por defecto (DefaultTableModel).
        // Se sobrescribe el metodo isCellEditable para hacer que todas las celdas de la tabla no sean editables.
        ...
        for (int i = 0; i < users.size(); i++) {
            ...
            // Establece el valor del estatus del usuario (probablemente obtenido a través de un metodo 'getStrEstatus()')
            // en la celda correspondiente de la fila actual (columna 3).
            model.setValueAt(user.getStrEstatus(), i, 3);
        }

        // Llama al metodo 'hideCol' para ocultar la columna con índice 0 (la columna del ID).
    ...
    // Metodo privado para obtener el objeto User seleccionado de la fila de la tabla.
    private User getUserFromTableRow() {
        ...
        try {
            ...
            // Llama al metodo 'getById' del UserDAO para obtener el objeto User correspondiente al ID obtenido de la tabla.
