...
public class LoginForm extends JDialog {
    ...
    public LoginForm(MainForm mainForm){
        ...
        addWindowListener(new WindowAdapter() { // Agrega un WindowListener a esta ventana para escuchar eventos de ventana.
            @Override
            public void windowClosing(WindowEvent e) { // Sobreescribe el metodo 'windowClosing'. Este metodo se llama cuando el usuario intenta cerrar la ventana (por ejemplo, haciendo clic en el botón de cerrar).
    ...
    private void login() {
        try{
            ...
            User userAut = userDAO.authenticate(user); // Llama al metodo 'authenticate' del objeto 'userDAO' para verificar las credenciales del usuario contra la base de datos. El resultado (un objeto User si la autenticación es exitosa, o null si falla) se almacena en 'userAut'.
            ...
            if(userAut != null && userAut.getId() > 0 && userAut.getEmail().equals((user.getEmail()))){
                this.mainForm.setUserAuthenticated(userAut); // Si la autenticación es exitosa, establece el usuario autenticado en el formulario principal ('mainForm'). Esto permite que el formulario principal acceda a la información del usuario logueado.
